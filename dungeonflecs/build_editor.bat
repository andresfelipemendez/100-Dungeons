...
for %%f in ("%EDITOR_SRC_PATH%\*.cpp") do (
    if "!SOURCE_FILES!"=="" (
        set "SOURCE_FILES=%%f"
    ) else (
...
clang++ -shared -std=c++17 -g -D_ITERATOR_DEBUG_LEVEL=0 -D_MT -D_DLL -DIMGUI_DEFINE_MATH_OPERATORS^
    -I"%INCLUDE_PATH%" -I"%CORE_INCLUDE_PATH%" -I"%INCLUDE_PATH%" -I"%EXTERNAL_INCLUDE_PATH%" -I"%FASTGLTF_INCLUDE_DIR%" -I"%GLM_INCLUDE_DIR%"^
    -I"%GLAD_INCLUDE_DIR%" -I"%IMGUI_INCLUDE_DIR%" -I"%IMGUI_INCLUDE_BACKENDS_DIR%" -I"%TOML_INCLUDE_DIR%" ^
    -I"%GLFW_INCLUDE_DIR%" ^
    !SOURCE_FILES! -o "%OUTPUT_PATH%\editor.dll" ^
...
if %errorlevel% neq 0 (
    ...
    powershell -Command "Write-Host 'Editor Compilation succeeded.' -ForegroundColor Green"
)
