...
./test.out
gcc test_e2e.c -o test_e2e.out -lpthread
./test_e2e.out
# fuzz under the sanitizers; no mallocs of our own, leak detection just
# flags dlopen internals
gcc -fsanitize=address,undefined -g test_fuzz.c -o test_fuzz.out -lpthread
ASAN_OPTIONS=detect_leaks=0 ./test_fuzz.out
gcc -fsanitize=address,undefined -g test_stress.c -o test_stress.out -lpthread
