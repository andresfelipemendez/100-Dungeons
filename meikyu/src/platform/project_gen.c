...
static b32 kaji_emit_and_write(const Project *p, b32 *changed) {
    ...
    char includes[2560];
    /* lib/horu, lib/tsumami, lib/henshu, lib/mono: a project may pull in these
       game-side libs (e.g. the henshu editor, the mono entity model) which
       include each other's headers by bare name. lib/seni covers
       seni_annotations.h (the SENI_DEFAULTs in their state headers). */
    snprintf(includes, sizeof(includes),
             "%s src \"%s/src\" \"%s/src/editor\" \"%s/lib/ito\" \"%s/cgltf\" "
             "\"%s/stb\" \"%s/clay\" \"%s/lib/seni\" \"%s/lib/horu\" "
             "\"%s/lib/tsumami\" \"%s/lib/henshu\" \"%s/lib/mono\" "
             "\"%s" SDL_MINGW "/include\"",
    ...
    emit("deny_include \"%s/lib/mono\"     dodai\n", g_engine);
