...
void watch_core() {
	...
	if (hDir == INVALID_HANDLE_VALUE) {
		std::cerr << "CreateFile failed for directory: src/core ("
...
void reload_loop() {
	...
	if (init == NULL || stop == NULL) {
		fprintf(
			stderr,
			"Failed to get function address: init or stop, error code: %lu\n",
			GetLastError());
		FreeLibrary(hLib);
		return;
	}
		...
		if (reloadEngineFlag.load()) {
			...
			if (hLib) {
				...
				if (result == 0) {
					...
					std::cerr << "FreeLibrary failed: Error code " << error
							  << std::endl;
				} else {
					// FreeLibrary succeeded
					std::cout << "FreeLibrary called successfully."
							  << std::endl;
					hLib = nullptr;
				}
			...
			if (checkModule == NULL && GetLastError() == ERROR_MOD_NOT_FOUND) {
				...
				std::cerr << "DLL is still loaded, trying to unload again..."
						  << std::endl;
				...
				while (checkModule != NULL) {
					...
					if (retryResult == 0) {
						DWORD error = GetLastError();
						std::cerr << "FreeLibrary retry failed: Error code "
								  << error << std::endl;
						break;
					} else {
						std::cout << "Retrying FreeLibrary successful."
								  << std::endl;
					}
				...
				if (checkModule == NULL &&
					...
					std::cerr << "After multiple attempts, DLL is still "
								 "loaded. Giving up."
							  << std::endl;
				}
			}
			...
			for (int attempt = 0; attempt < retries; ++attempt) {
				if (copy_dll("core", copiedCoreDllPath,
							 sizeof(copiedCoreDllPath))) {
					copied = true;
					break;
				} else {
					std::cerr << "Could not copy DLL, retrying... Attempt "
			...
			if (!copied) {
				std::cerr << "Failed to copy DLL after several attempts. "
							 "Aborting reload.\n";
				continue;
			}
			...
			if (hLib == NULL) {
				std::cerr << "Failed to load library: " << copiedCoreDllPath
			...
			if (init == NULL || stop == NULL) {
				std::cerr << "Failed to get function address: init or stop, "
							 "error code: "
						  << GetLastError() << "\n";
				FreeLibrary(hLib);
				hLib = nullptr;
